...
use traits::vamm::{Vamm as VammTrait, VammConfig, MINIMUM_TWAP_PERIOD};
...
fn should_fail_if_twap_period_is_less_than_minimum() {
    ...
    ExtBuilder::default().build().execute_with(|| {
        assert_noop!(
            TestPallet::create(&vamm_state),
            Error::<MockRuntime>::FundingPeriodTooSmall
        );
    });
}
...
proptest! {
    ...
    fn should_succeed_correctly_returning_vamm_id(
        ...
        ExtBuilder::default().build().execute_with(|| {
            ...
            // TODO(Cardosaum): Add checks for other files in Twap value
