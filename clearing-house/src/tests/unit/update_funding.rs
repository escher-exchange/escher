use crate::{
    mock::{
        ...
        unit::{
            ...
            runtime::{
                Assets as AssetsPallet, ExtBuilder, MarketId, Origin, Runtime,
                System as SystemPallet, TestPallet, Vamm as VammPallet, MINIMUM_PERIOD_SECONDS,
            },
        },
    },
    tests::unit::{
...
fn should_update_oracle_and_vamm_twaps() {
    ...
    with_market_context(ext_builder, config, |market_id| {
        ...
        assert_eq!(
            VammPallet::get_twap(vamm_id, AssetType::Base).unwrap(),
            1.into()
        );
    });
}
